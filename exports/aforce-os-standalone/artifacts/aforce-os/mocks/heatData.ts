...
import { HEAT_BANDS, bandForScore, evaluateHeatRisk } from "../services/heatRiskEngine";

// ─── Sample inputs by band ──────────────────────────────────────────────────
export const SAMPLE_INPUTS: Record<HeatRiskBand, HeatSignalInput> = {
  STABLE: {
    hydrationScore: 88,
    recentFluidOz: 18,
  ...
  HIGH_RISK: {
    ...
    humidityPct: 60,
    sunExposure: 0.8,
    continuousActiveMin: 58,
    activityIntensity: 0.78,
    heartRateBpm: 168,
    hrRecoveryDelaySec: 24,
    sweatLossOzPerHr: 28,
    bodyWeightLbs: 185,
    recoveryMomentum: 0.4,
    symptoms: ["headache", "cramping"],
    urineSignal: 5,
    energyState: "low",
    sleepDeficitHrs: 1,
    recentHeatEvent: false,
...
export const BAND_LIST: HeatBandDisplay[] = HEAT_BANDS;
...
interface MockAthlete {
  ...
  jerseyNumber: number;
  position: string;
  /** Which preset signal pattern the athlete is currently on. */
  pattern: HeatRiskBand;
  trend: "rising" | "falling" | "steady";
}

const MOCK_ROSTER: MockAthlete[] = [
...
function alertStateForBand(band: HeatRiskBand): TeamHeatAlertState {
  switch (band) {
    case "STABLE":
    case "ELEVATED":
      return "WATCH";
...
function coachActionFor(band: HeatRiskBand, name: string): string {
  switch (band) {
    case "STABLE":
...
export function buildMockRoster(): TeamHeatAthlete[] {
  return MOCK_ROSTER.map((m) => {
    const score = evaluateHeatRisk(SAMPLE_INPUTS[m.pattern]);
    const band = bandForScore(score.score).band;
    return {
      id: m.id,
      name: m.name,
      ...
      band,
      trend: m.trend,
      hydrationScore: SAMPLE_INPUTS[m.pattern].hydrationScore,
      alertState: alertStateForBand(band),
      coachAction: coachActionFor(band, m.name.split(" ")[1] ?? m.name),
      minutesSinceLastIntake: SAMPLE_INPUTS[m.pattern].minutesSinceLastIntake,
    };
  });
}

export function buildMockAlertFeed(): TeamHeatAlert[] {
  ...
  return roster
    ...
    .map((a, idx) => ({
      ...
      loggedAt: new Date(Date.now() - idx * 90_000).toISOString(),
    }));
}

...
SLICE 4a+4b REVIEWED (revenue-guardian, 2026-07-26): FIX-FIRST defect (cadence-unaware price lookup → cross-cadence mischarge) FIXED in PR #393 (lookup pinned to exact amount+interval, fail-safe fallback; annual parity assertion added). GATE CLEARED (founder Stripe audit 2026-07-26) → cadence picker SHIPPED (PR #398): explicit $20/mo vs $200/yr choice, no silent cadence, web fallback. PASS-3 slices 1,2,3,4a,4b ALL COMPLETE. PASS-3 PLAN 100% COMPLETE (PR #400 merged 2026-07-26): slice 4c bridge shipped through BOTH gates — revenue-guardian FIX-FIRST (selling-plan allowlist, terminal cancel, Clerk verified-email trust root) + cybersecurity SIGN-OFF (5 vectors PASS, read-time plan clamp applied). BEFORE the bridge is live: deploy aforce_web_entitlements (R-21 batch) + set SHOPIFY_WEBHOOK_SECRET on Railway BEFORE the Shopify webhook activates (auto-disable after ~48h non-2xx). Follow-ups tracked: null-period-end reconciliation, cross-rail double-billing notice, fill-once email staleness (documented residual).

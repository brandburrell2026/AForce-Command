...
- **Cruise Mode:** Premium **guest-facing** add-on for hydration intelligence at sea. Crew / staff get personalized hydration support through Social Mode rather than Cruise Mode — only guest demo profiles (Pool-day, Excursion) appear in the on-screen profile picker, and the Crew Performance Mode section has been removed. Ship Environment pulls live conditions from OpenWeather via `GET /api/cruise/environment?port=<id>` (api-server route `routes/cruise.ts`, 10-min in-memory cache, deterministic Caribbean fallback when upstream is unavailable). The operator-facing Fleet Dashboard (`FLEET_DEMO` in `services/cruiseModeService.ts`) is intentionally retained as a B2B view with KPI strip, ship switcher across Royal Caribbean / Carnival / Virgin / Disney, and per-ship department compliance.

...
- **Home expansion:** the score orb is retained as the **Readiness Score** hero (thin `READINESS SCORE` eyebrow; engine machinery unchanged). Appended below: the Hydration Status ring. (Owner trim, June 2026: the Daily Ritual rail, Today's Protocol, Streak, Athlete Mode progress, and Membership cards were removed from the Home surface. Their components and the `utils/homeDashboard` derivations remain in the codebase — simply no longer rendered — so the trim is reversible.)

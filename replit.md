...
- `services/__tests__/beverageComparisonEngine.test.ts` (5 tests) pins the rubric: AForce scores >75, beats every legacy sugar drink, alkaline lift > acidic competitors, clean-label flags artificial sweeteners (Propel), and the per-metric winner map always covers all 5 keys.
- **Scan → compare deep link.** When a HydroScan recognizes a non-AForce product (barcode, QR, or manual search), the result region surfaces a "COMPARE <BRAND> WITH AFORCE" CTA that deep-links into `/hydroscan-compare?competitor=<id>` with the competitor pre-selected — no second tap to find it in the picker. Wiring lives in `competitorIdForScannedProduct()` (in `data/beverageCompetitors.ts`), which maps scan-catalog ids → CompetitorIds; AForce SKUs return `null` (no self-compare) and unrecognized brands return `undefined` (CTA hidden, no orphan navigation). HydroScanCompareScreen reads the param via `useLocalSearchParams` and validates against the competitor list so a malformed URL falls back to the picker. Three new tests cover the mapping (known competitors, AForce SKUs, unknown brands). Tests: 80 → 88.

...
- **Format:** 28 React slides served at `/aforce-pitch/slide{1..28}` via the standard slides scaffold (manifest + per-slide TSX components, w-screen h-screen, vw/vh-only sizing). Validate with `pnpm --filter @workspace/aforce-pitch run validate-slides`.

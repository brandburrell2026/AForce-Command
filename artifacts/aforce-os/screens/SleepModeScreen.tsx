...
function SleepModeRedesign() {
  ...
  React.useEffect(() => {
    if (!isFiniteNum(sleepLastNight)) return;
    setSevenNightAvg((prev) => {
      const next = prev == null ? sleepLastNight : prev + (sleepLastNight - prev) / 7;
  ...
  const onToggleChecklist = (id: ChecklistItemDef['id']) => {
    setCompleted((prev) => {
      ...
      else { next.add(id); haptic('light'); if (next.size === view.checklist.totalCount) haptic('success'); }
      return next;
    });
  };
  const onPrimaryCta = () => {
    haptic(view.checklist.completedCount === view.checklist.totalCount ? 'success' : 'light');
    // Presentation-only: the protocol is the checklist itself. No score mutation,
    // no fabricated completion. A fully-checked protocol simply reads "complete".
  };

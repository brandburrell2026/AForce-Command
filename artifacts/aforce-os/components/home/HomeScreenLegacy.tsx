/**
 * Home (legacy) — pre-Phase-2 Hydration Control System screen.
 *
 * RC-1 Wave-3 P1 perf fix (audit P2-7): relocated verbatim out of
 * `./index.tsx` so it can be `React.lazy`-loaded instead of statically
 * imported. `spec_home: true` is the default (`featureFlags/flags.ts`), so
 * in production this branch is unreachable — but `index.tsx` used to
 * statically import this entire component tree (CommandConsole,
 * EntryActions, HomeDashboard, MetabolicReadinessZone, PerformanceAgeZone,
 * VoiceCheckInZone, ActivationJourneyZone, AIVideoPlayer, StatusPulseOrb,
 * FlavorPickerModal, etc.) anyway, so Metro evaluated all of it on every
 * cold start regardless of the flag. Moving the render path into its own
 * module lets `index.tsx` `React.lazy(() => import('./HomeScreenLegacy'))`
 * it instead, so Metro only evaluates this module the moment the flag
 * branch actually renders.
 *
 * NOTHING in this file was changed beyond the move itself — no JSX, no
 * logic, no styles were edited, added, or removed (see the PR body for the
 * diff evidence). Deletion of the legacy Home entirely is a separate,
 * pending founder decision (CLAUDE.md working agreement) — this is a
 * lazy-load-only relocation, not a rewrite or a removal.
 */

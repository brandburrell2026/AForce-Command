...
export function AdvancedVisualIntelligenceScreen({ onBeginCapture }: { onBeginCapture?: () => void }) {
  ...
  const title = view === 'consent' ? 'Before you continue.' : view === 'privacy' ? 'Your image does not stay.' : view === 'acknowledged' ? 'You are in control.' : view === 'guidance' ? 'Prepare, then decide.' : view === 'permissionDenied' ? 'Camera access is off.' : view === 'qualityUnknown' ? 'Do not force a result.' : view === 'baselineUnavailable' ? 'Your baseline is not ready.' : view === 'consentRevoked' ? 'Consent has been withdrawn.' : 'See today.\nCompare over time.';

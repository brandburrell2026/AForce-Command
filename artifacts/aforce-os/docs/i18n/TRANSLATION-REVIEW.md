...
| `profile.v2.apple_updated_confirmation` | `components/profile/AppleHealthRefreshControl.tsx` (RC-2, TestFlight build 45 founder-reported fix) | `"Updated just now"` | de, es, fr, it, pt, ar, hi, ja, ko, zh | New key — the Apple Health refresh completion confirmation (fires on every successful re-read, including byte-identical data; states a re-read succeeded, never that new data arrived). Same `profile.v2.*`-is-English-only-across-all-10 situation as the two rows above — no existing equivalent phrase to carry over. |

...
| `home.v2.freshness.just_now` / `.minutes_ago` / `.hours_ago` / `.days_ago` / `.unavailable` | `components/home/homeFreshness.ts` + `components/home/HomeFreshnessLabel.tsx` (RC-2 ruling E, item 1 — truthfulness fix) | `"Checked just now"` / `"Checked {{count}}m ago"` / `"Checked {{count}}h ago"` / `"Checked {{count}}d ago"` / `"Awaiting first sync"` | de, es, fr, it, pt, ar, hi, ja, ko, zh | New keys, replacing the old flat `home.v2.freshness` = `"Updated just now"` (a truthfulness bug — rendered unconditionally with no timestamp). Verb deliberately set to "Checked", not "Updated" — see the `profile.v2.apple_updated_confirmation` row immediately above: build 47 already fought this exact battle for the identical reason (a `fetchedAt` read does not imply the value changed), and reverting to "Updated" here would re-open it. Same `home.v2.*`-is-English-only-across-all-10 situation as the `home.live_status.*` rows above — no existing equivalent phrase to carry over. |

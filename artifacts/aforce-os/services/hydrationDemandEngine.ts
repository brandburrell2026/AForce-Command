 ...
 * module can run in tests, in the engine reducer, and in the future
 * Intelligence Layer prompt builder without instantiating any UI.
 *
 ...
 *
 * Phase 1: pure module, no visible surface consumes it. Gated by
 * `spec_demand_engine` (default OFF in prod, ON in DEMO_ALL_ON).
 */
...
export function computeHydrationDemand(
  ...
  const remainingOz = Math.max(0, Math.round(targetOz - consumedOz - cycleVolume));

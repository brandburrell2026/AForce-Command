...
| **Night Out Protocol** (current "Social Mode") | **Partially Built; Blocked for Night-Out public exposure** — see `AFORCE_OS_NIGHT_OUT_PROTOCOL_SPEC.md` §15/§19 (naming NO-1, Protocol placement NO-2, BAC/hangover NO-5/NO-6, age/regional NO-8, flag/entitlement NO-10). **NO-a landed:** `night_out_enabled` flag (Founder/Internal-Preview, OFF in prod) + canonical session-state model (`services/nightOut/sessionState.ts`) reconciled onto `SocialModeState`. **NO-a.1 landed:** internal flag isolation — generic client "unlock all" cannot enable it (restricted-flag clamp `demoUnlockAllFlags`), plus an authorization resolver where the flag is necessary-but-not-sufficient (approved `DEMO_MODE` context required); `services/nightOut/access.ts`. 26 tests. Server-side authorization still outstanding (NO-10). **NO-b landed:** public rename Social Mode → NIGHT OUT (AForce Night Out Protocol / Private Evening Protocol); authorized entry inside the Protocol tab; canonical gated `/night-out` route; social-v2/social-legacy de-discovered via safe redirects; Home pip/banner/entry-tile gated behind authorization; five tabs preserved; no scoring/alcohol/persistence/entitlement change; 43 tests. **NO-c landed:** Water-First command experience (HYDROSTATE→NOW→NEXT→LATER) at gated `/night-out` — authoritative-timestamp timer with background/force-close restoration (local only; no cross-device), COMPLETE WATER via approved `logIntake`, confidence/freshness display, Adjust (approved amounts)/Not Now (no penalty), reduced-motion; 72 tests. No scoring/alcohol/schema/entitlement change. Real-device screenshots pending. |

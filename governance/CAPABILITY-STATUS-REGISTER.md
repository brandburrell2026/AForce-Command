...
| **Stage 3 — §42 Intelligence Language & Claims Gate** | **Partially Built — block-severity vocabulary LIVE at runtime (Wave-2 #732, 2026-08-12)** | production (seam scan); full gate local-only | none | public (as suppression only) | full `evaluateClaim()` ClaimCandidate path still has **no caller** (needs provenance adapter); locale-suppression rule not enabled (founder decision W2-N2) | `types/claimGate.ts`, 4 modules under `utils/intelligence/languageGate/` + `runtimeClaimScan.ts`. 62 gate tests + 27 seam/parity/lint tests passing. Block-severity concepts enforced at speak()/voice-templates/scan-coach/command-overlays/smart-capture//voice/tts; static lint across all 12 locale files. English-only validated. |

...
>
> **Update 2026-07-31 (ter) — citation corrections APPLIED (performance-scientist pass).** All 3 CHECK-PRIMARY items are fixed across 11 locales + the legacy screen + engine/type comments: Baker **"Table 2" → "Figure 2"**; the fabricated "mean ≈ 50 mmol/L" range replaced with source-accurate **"≈ 10–90 mmol/L (≈ 230–2070 mg/L)"** (matches the app's own SODIUM_BANDS); Maughan pages **31–42 → 20(Suppl 2):59–69**; all **§C/§G/§F** shorthand replaced with paper-level ACSM/Sawka coordinates. `cite_estimate_body` kept on Baker 2017 (the 2019 by-sport numbers don't match the app's values → citing 2019 would create a *fresh* mismatch). **New follow-up (not fixed — needs a value↔source decision):** the per-sport tables in `data/sweatSports.ts` have their own provenance defects — Baker "Table 1" is methodological (not sport-rates); the soccer citation "Br J Sports Med 41:e1" appears wrong (real paper: Maughan et al. 2007 *IJSNEM* 17(6):583–594); and the football value **2.14 L/h** matches **Godek 2005** (*Br J Sports Med* 39(4):205–211), not the cited Godek 2010 (which reports 1.42/1.98/2.16). Fixing these means either re-citing to the paper that matches each value or changing a data value (functional) — left for the performance-scientist/ml-engineer.

...
| **ER-5** | **In-app citation numbers** — `en.json:1794–1810` (Sawka 2007, Baker 2017, Cheuvront 2014, Maughan & Shirreffs 2010) | LIVE | Verification done (`CR-1-ER5-CITATION-VERIFICATION.md`): **1 SUPPORTED · 3 CHECK-PRIMARY · 2 MISMATCH**; arithmetic all PASS | performance-scientist (done) → 2 citation fixes | **VERIFIED — 2 MISMATCH fixed; primary check found 2 citation-accuracy fixes** | MISMATCH copy defects fixed. **Primary-source check (2026-07-31):** **Baker "Table 2"** is a variability-*factors* table, not the sodium ranges, and the range runs above Baker 2017's ~10–70 mmol/L → fix the pointer / re-source (poss. Baker 2019); **Maughan & Shirreffs 2010 is 20(Suppl 2):59–69, not 31–42**. Cheuvront/Sawka coords correct; "§C/§G/§F" are internal shorthand to replace. Citation-accuracy fixes (need the performance-scientist to pick source/pages); physiological directions sound, no claim risk |

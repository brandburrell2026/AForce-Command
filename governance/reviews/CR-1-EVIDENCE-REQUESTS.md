...
| **ER-5** | **In-app citation numbers** — `en.json:1794–1810` (Sawka 2007, Baker 2017, Cheuvront 2014, Maughan & Shirreffs 2010) | LIVE | Verification done (`CR-1-ER5-CITATION-VERIFICATION.md`): **1 SUPPORTED · 3 CHECK-PRIMARY · 2 MISMATCH**; arithmetic all PASS | performance-scientist (done) → citation fixes APPLIED | **VERIFIED + citation fixes APPLIED 2026-07-31** | MISMATCH copy defects fixed; then a primary-source check + performance-scientist pass **applied all 3 citation corrections** (Baker "Table 2"→"Figure 2" + range → 10–90 mmol/L; Maughan → 20(Suppl 2):59–69; §C/§G/§F → paper-level coords) across 11 locales + screen + comments. **One follow-up open:** `data/sweatSports.ts` per-sport provenance (Baker "Table 1", soccer citation, Godek 2005-vs-2010 / value 2.14) — a value↔source decision (may change a data value), left for performance-scientist/ml-engineer. No claim risk |

...
>
> **Update 2026-07-31:** ER-1, ER-2, ER-3 resolved — ER-1/ER-2 **held** (claims removed pending evidence, like the marketing fix), ER-3 **genericized** (PR #413). **ER-5 verification is done** (`CR-1-ER5-CITATION-VERIFICATION.md`) and surfaced **2 must-fix copy defects** (the "±15%" unsupported-validation line; a "Baker 2017" attribution that should credit Maughan 2007 / Godek 2010) plus 3 primary-source checks — these are open. ER-4 stays behind its dark flag.
...
| **ER-5** | **In-app citation numbers** — `en.json:1794–1810` (Sawka 2007, Baker 2017, Cheuvront 2014, Maughan & Shirreffs 2010) | LIVE | Verification done (`CR-1-ER5-CITATION-VERIFICATION.md`): **1 SUPPORTED · 3 CHECK-PRIMARY · 2 MISMATCH**; arithmetic all PASS | performance-scientist (done) → copy fixes + primary-source checks | **VERIFIED 2026-07-31 — defects OPEN** | 2 must-fix copy defects: **(1)** the "±15%" line is an unsupported *validation* claim (no study/test in repo); **(2)** per-sport rates credited to "Baker 2017" actually come from Maughan 2007 / Godek 2010. Plus 3 CHECK-PRIMARY (Baker Table 2, Cheuvront §C, Sawka §G coords) + a minor BSA constant |
...
- **Before launch (gate live claims):** **ER-5 defects** — the 2 MISMATCH copy fixes (±15% line, per-sport attribution) must ship; the 3 CHECK-PRIMARY items need the actual papers confirmed. ER-1/ER-2 **held**, ER-3 **genericized**. Attach the CoA (ER-1) / PK data (ER-2) to *restore* those claims later.

...
>
> **Two of these gate claims that are LIVE in the app right now** (ER-1, ER-2) and one gates live copy pending a decision (ER-3). Those are **substantiate-or-remove before launch** — the copy currently ships an unsubstantiated claim.
...
| **ER-3** | **Named-competitor characterizations** — `cmp_profile_gatorade/lmnt/liquid_iv` (`en.json:1785–1787`): "Sugar-driven · table salt", "Salt-bomb · no minerals", "Sugar + salt · no structuring" | **LIVE — pending decision + evidence** | **Current, dated, sourced published nutrition facts** (sodium, sugar, mineral content) for each named product, supporting every stated/implied contrast — incl. the "no minerals"/"no structuring" absence claims | Founder (sourcing) → performance-scientist + regulatory reviewer (Lanham) | OPEN | Keeping the named-competitor rows. If not sourced ⇒ genericize to categories ("high-sodium mixes", "sugar-based sports drinks") or remove. **Note:** "no structuring" is doubly unsupported while "structuring" itself is unproven |
...
- **Before launch (gate live claims):** ER-1, ER-2, ER-3, ER-5. Each is either substantiated (doc attached + validated) or the claim is removed/reworded — no "pending" claims ship.

...
>
> **Update 2026-07-31:** ER-1, ER-2, ER-3 are all resolved — ER-1/ER-2 **held** (the unsubstantiated claims were removed from copy pending evidence, like the marketing fix), ER-3 **genericized** (PR #413). The only remaining live item is **ER-5** (an internal citation number-to-source verification). ER-4 stays behind its dark flag. No unsubstantiated claim currently ships.
...
- **Before launch (gate live claims):** **ER-5** only (internal citation verification). ER-1/ER-2 **held** (claims removed pending evidence), ER-3 **genericized** — none ship an unsubstantiated claim now. Attach the CoA (ER-1) / PK data (ER-2) to *restore* those claims later.

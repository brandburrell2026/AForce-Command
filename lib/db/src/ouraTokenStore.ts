/**
 * Oura Ring OAuth2 token store — server-side, per-user persistence.
 *
 * Faithful mirror of `whoopTokenStore.ts` / `garminTokenStore.ts`. The
 * three stores are intentionally shape-identical so the Oura token
 * manager can be a near-drop-in port of WHOOP's/Garmin's.
 *
 * `expiresAt` semantics:
 *   - In-memory / manager: epoch ms (cheap arithmetic).
 *   - Postgres boundary: `timestamptz`. Converted at the store boundary.
 *
 * Hidden-infra: nothing writes here until OURA_CLIENT_ID +
 * OURA_CLIENT_SECRET + OURA_OAUTH_REDIRECT_URI are all configured (the
 * OAuth router is only mounted then). With nothing configured the
 * schema simply sits unused.
 */

import { eq, sql } from "drizzle-orm";
import type { NodePgDatabase } from "drizzle-orm/node-postgres";
import { aforceOuraTokens } from "./schema/aforce";

/** Same shape as the WHOOP/Garmin token bundle. `expiresAt` is epoch ms. */
 ...
 */
export interface OuraTokenStoreLogger {
  warn: (obj: Record<string, unknown>, msg?: string) => void;
}

export interface DrizzleOuraTokenStoreOptions {
  /**
   ...
   * same `OURA_TOKEN_ENCRYPTION_KEY` env var to keep this invariant.
   *
...
export interface OuraTokenStore {
  read(): Promise<OuraTokens | null>;
  write(t: OuraTokens): Promise<void>;
  clear(): Promise<void>;
}

/** In-memory store. Use for tests, local dev, and as a fallback when
 *  DB is unavailable. */
export function createInMemoryOuraTokenStore(
  seed: OuraTokens | null = null,
): OuraTokenStore {
  let current: OuraTokens | null = seed;
  return {
    async read() {
      return current;
    },
 ...
 * batched; returns the number of rows updated. Mirrors the WHOOP/Garmin
 * backfill helper exactly.
 */
export async function backfillOuraTokenEncryption(
  ...
  if (typeof encryptionKey !== "string" || encryptionKey.trim() === "") {
    throw new Error(
      "backfillOuraTokenEncryption: encryptionKey must be a non-empty string",
    );
  }
  if (!Number.isFinite(batchSize) || batchSize <= 0) {
    throw new Error(
      "backfillOuraTokenEncryption: batchSize must be a positive number",
    );
  }
  const result = await db.execute<{ user_id: string }>(sql`
    update aforce_oura_tokens
       set access_token_enc  = coalesce(
           ...
           updated_at        = now()
     where user_id in (
       select user_id from aforce_oura_tokens
 ...
 * WHOOP/Garmin status helper exactly (single SQL roundtrip via FILTER
 * aggregates).
 */
export async function getOuraTokenEncryptionStatus(
  db: NodePgDatabase<Record<string, unknown>>,
): Promise<{
  total: number;
  encrypted: number;
 ...
 */
export function createDrizzleOuraTokenStoreForUser(
  db: NodePgDatabase<Record<string, unknown>>,
  userId: string,
  opts: DrizzleOuraTokenStoreOptions = {},
): OuraTokenStore {
  if (!userId) {
    throw new Error(
      "createDrizzleOuraTokenStoreForUser: userId must be non-empty",
    );
  }
  ...
  return {
    async read() {
      if (encryptionKey) {
        try {
          ...
          log?.warn(
            ...
            "ouraTokenStore: pgp_sym_decrypt failed, falling back to plaintext column",
          );
        }
      }
      ...
      const rows = await db
        ...
        .where(eq(aforceOuraTokens.userId, userId))
        .limit(1);
      const row = rows[0];
      if (!row) return null;
      return {
    ...
    async write(t) {
      if (encryptionKey) {
        await db.execute(sql`
          insert into aforce_oura_tokens
      ...
      await db
        ...
        .onConflictDoUpdate({
          target: aforceOuraTokens.userId,
          set: {
            accessToken: t.accessToken,
            refreshToken: t.refreshToken,
            expiresAt: new Date(t.expiresAt),

/**
 * Garmin Connect OAuth2 token store — server-side, per-user persistence.
 *
 * Faithful mirror of `whoopTokenStore.ts` (codename "il" on the server).
 * The two stores are intentionally shape-identical so the Garmin token
 * manager can be a near-drop-in port of the WHOOP one.
 *
 * `expiresAt` semantics:
 *   - In-memory / manager: epoch ms (cheap arithmetic).
 *   - Postgres boundary: `timestamptz`. Converted at the store boundary.
 *
 * Architecture lock: DORMANT / hidden-infra. Nothing writes here until
 * GARMIN_CLIENT_ID + GARMIN_CLIENT_SECRET + GARMIN_OAUTH_REDIRECT_URI are
 * all configured (the OAuth router is only mounted then). With nothing
 * configured the schema simply sits unused.
 */

import { eq, sql } from "drizzle-orm";
import type { NodePgDatabase } from "drizzle-orm/node-postgres";
import { aforceGarminTokens } from "./schema/aforce";

/** Same shape as the WHOOP token bundle. `expiresAt` is epoch ms. */
export interface GarminTokens {
  accessToken: string;
  refreshToken: string;
  /** Epoch ms when the access token expires. */
  expiresAt: number;
  /** Space-separated scopes the user granted. Optional — null when
   *  Garmin doesn't echo `scope` back. */
  scope?: string | null;
}
 ...
 */
export interface GarminTokenStoreLogger {
  warn: (obj: Record<string, unknown>, msg?: string) => void;
}

export interface DrizzleGarminTokenStoreOptions {
  /**
   ...
   * same `GARMIN_TOKEN_ENCRYPTION_KEY` env var to keep this invariant.
   *
   ...
   * read still succeeds.
   *
   * **Dormant-infra contract**: unset (default) = unchanged behavior.
...
export interface GarminTokenStore {
  read(): Promise<GarminTokens | null>;
  write(t: GarminTokens): Promise<void>;
  clear(): Promise<void>;
}

/** In-memory store. Use for tests, local dev, and as a fallback when
 *  DB is unavailable. */
export function createInMemoryGarminTokenStore(
  seed: GarminTokens | null = null,
): GarminTokenStore {
  let current: GarminTokens | null = seed;
  return {
    async read() {
      return current;
    },
 ...
 * batched; returns the number of rows updated. Mirrors the WHOOP
 * backfill helper exactly.
 */
export async function backfillGarminTokenEncryption(
  ...
  if (typeof encryptionKey !== "string" || encryptionKey.trim() === "") {
    throw new Error(
      "backfillGarminTokenEncryption: encryptionKey must be a non-empty string",
    );
  }
  if (!Number.isFinite(batchSize) || batchSize <= 0) {
    throw new Error(
      "backfillGarminTokenEncryption: batchSize must be a positive number",
    );
  }
  const result = await db.execute<{ user_id: string }>(sql`
    update aforce_garmin_tokens
       set access_token_enc  = coalesce(
           ...
           updated_at        = now()
     where user_id in (
       select user_id from aforce_garmin_tokens
 ...
 * WHOOP status helper exactly (single SQL roundtrip via FILTER
 * aggregates).
 */
export async function getGarminTokenEncryptionStatus(
  db: NodePgDatabase<Record<string, unknown>>,
): Promise<{
  total: number;
  encrypted: number;
 ...
 */
export function createDrizzleGarminTokenStoreForUser(
  db: NodePgDatabase<Record<string, unknown>>,
  userId: string,
  opts: DrizzleGarminTokenStoreOptions = {},
): GarminTokenStore {
  if (!userId) {
    throw new Error(
      "createDrizzleGarminTokenStoreForUser: userId must be non-empty",
    );
  }
  ...
  return {
    async read() {
      if (encryptionKey) {
        try {
          ...
          log?.warn(
            ...
            "garminTokenStore: pgp_sym_decrypt failed, falling back to plaintext column",
          );
        }
      }
      ...
      const rows = await db
        .select()
        .from(aforceGarminTokens)
        .where(eq(aforceGarminTokens.userId, userId))
        .limit(1);
      const row = rows[0];
      if (!row) return null;
      return {
    ...
    async write(t) {
      if (encryptionKey) {
        await db.execute(sql`
          insert into aforce_garmin_tokens
      ...
      await db
        ...
        .onConflictDoUpdate({
          target: aforceGarminTokens.userId,
          set: {
            accessToken: t.accessToken,
            refreshToken: t.refreshToken,
            expiresAt: new Date(t.expiresAt),
